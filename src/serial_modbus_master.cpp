...
int main(int argc, char** argv) {
  ...
  int cnt_wr_regs = 0, cnt_wr_coils = 0, cnt_rd_regs = 0, cnt_rd_coils = 0;
  int error_count = 0;
  ...
  for (int i = 0; i < steps; ++i) {
    ...
    {
      auto t0 = clock::now();
      int rc = mb.writeRegisters(ctx, 0, { uint16_t(rpm), uint16_t(angle_val) });
      auto t1 = clock::now();
      t_wr_regs += msd(t1 - t0);
      ++cnt_wr_regs;
      if (rc < 0) {
    ...
    // readRegisters 측정 (배터리 status: addr=10, nb=4)
    {
      auto t0 = clock::now();
      ...
      if (rc == 4) {
        ...
        std::cerr << "\n[Master] readRegisters(batt) failed at step " << i << "\n";
        ++error_count;
      }
    }

    // readCoils 측정 (충전기 오류비트: addr=4, nb=6)
    {
      auto t0 = clock::now();
      ...
      if (rc == 6) {
        ...
        std::cerr << "\n[Master] readCoils(err) failed at step " << i << "\n";
        ++error_count;
      }
    }
